...
# Select 10 random genes and 10 random transcription factors from the specified chromosome
random_genes = random.sample(
    list(genes[chromosome].keys()), min(10, len(genes[chromosome]))
)
random_tfacts = random.sample(
    list(tfact[chromosome].keys()), min(10, len(tfact[chromosome]))
)
...
print(f" 10 selected genes {chromosome}:")
pprint.pprint(selected_genes)
print(f" 10 selected TF {chromosome}:")
